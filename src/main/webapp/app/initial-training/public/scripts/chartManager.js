...
class ChartManager {
  ...
  #categoryLabels = ['danceability', 'duration', 'energy', 'wordiness', 'tempo', 'happiness'];
  ...
  constructor(trackObjects, chartManagerOptions) {
    if (trackObjects) {
      ...
      this.colorProfiles.art = trackObjects.map(trackObject => trackObject.color.join(', '));
  ...
  generateDataset() {
    return this.trackObjects.map((trackObject, index) => {
      ...
      const data = this.#categoryLabels.map(label => trackObject.features.relative[label]);
  ...
  Chart.register({
    ...
    afterUpdate(chart) {
      ...
      legends.forEach((item, index) => {
        ...
        const img = `<img src=${trackObject.trackData.albumArt} alt="album cover for ${trackObject}" class="album-cover img-fluid" 
            style="background: ${item.fillStyle}; border-color: ${item.strokeStyle}; border-width: 3px;
