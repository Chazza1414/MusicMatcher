...
const displayStatsButton = () => {
  ...
  function updateStatsLink() {
    ...
    if (selectedRadios.length > 0) {
      const selectedIds = selectedRadios.get().map(radioElem => radioElem.dataset.songId);

      // add all the track Ids with URI query syntax,
      // then remove the trailing & w/slice
      const statsQuery = selectedIds
        .reduce((partialQuery, trackId, index) => partialQuery + `track${index + 1}=${trackId}&`, '/stats?')
        .slice(0, -1);
