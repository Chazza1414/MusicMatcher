...
class DataManager {
  ...
  calculateMaxes() {
    ...
    Object.keys(this.trackObjects[0].features.absolute).forEach(key => (initMaxes[key] = { trackObject: null, value: 0 }));
  ...
  calculateMins() {
    ...
    Object.keys(this.trackObjects[0].features.absolute).forEach(key => (initMins[key] = { trackObject: null, value: Infinity }));
  ...
  calculateAvg() {
    ...
    Object.keys(this.trackObjects[0].features.absolute).forEach(key => (initAvg[key] = { value: 0 }));
    ...
    Object.keys(initAvg).forEach(key => (avgObject[key].value /= this.trackObjects.length));
  ...
  applyRelativeData() {
    ...
    this.trackObjects.forEach(trackObject => {
      ...
      Object.keys(absolute).forEach(key => {
        trackObject.features.relative[key] = absolute[key] / this.data.max[key].value;
      });
    });
  }
}
